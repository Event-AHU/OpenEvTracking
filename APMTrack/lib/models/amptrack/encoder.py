...
from lib.models.amptrack import fastitpn as fastitpn_module
from lib.models.amptrack import itpn as oriitpn_module
...
class EncoderBase(nn.Module):
    ...
    def forward(self, z, x, event_template_voxel, event_search_voxel):
        xs = self.body(z, x, event_template_voxel, event_search_voxel)
        return xs
    ...
    def __init__(self, name: str,
        ...
        if "fastitpn" in name.lower():
            encoder = getattr(fastitpn_module, name)(
                ...
                token_type_indicate=cfg.MODEL.BACKBONE.TOKEN_TYPE_INDICATE,
                pretrain_type = cfg.MODEL.PRETRAIN_TYPE,
                patchembed_init = cfg.MODEL.BACKBONE.PATCHEMBED_INIT
            )
            if "itpnb" in name:
                num_channels = 512
            elif "itpnl" in name:
...
def build_encoder(cfg):
    ...
    encoder = Encoder(cfg.MODEL.BACKBONE.TYPE, train_encoder,
