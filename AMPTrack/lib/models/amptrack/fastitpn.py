...
import numpy as np
from lib.models.layers.event_encoder import EventEncoder
from lib.models.layers.event_warp import EventFeatureWarper, DepthwiseSeparableConv3D
from lib.models.layers.diff_attn import AttentionTransformer
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.event_encoder = EventEncoder(out_dim=512)
        self.eventfeat_warp = EventFeatureWarper(in_channels=512)
        self.dwcnn = DepthwiseSeparableConv3D(channels=512)
        
        self.scale1 = [0, 1, 2, 3, 4]
        # self.scale2 = [0, 2, 4]
        
        self.multi_aggblocks = AttentionTransformer(dim=512, num_heads=8, hidden_dim=768, num_layers=2, drop_path=0.1)
    ...
    def forward_features(self, z, x, event_template_voxel, event_search_voxel):
        ...
        if not self.convmlp and self.stop_grad_conv1:
            x = x.detach() * 0.9 + x * 0.1
            # event_x = event_x.detach() * 0.9 + event_x * 0.1
        ...
        for blk in self.blocks[:-self.num_main_blocks]:
            ...
            # event_z = checkpoint.checkpoint(blk, event_z) if self.grad_ckpt else blk(event_z)  # bn,c,h,w
            # event_x = checkpoint.checkpoint(blk, event_x) if self.grad_ckpt else blk(event_x)  # bn,c,h,w
            
        x = x.flatten(2).transpose(1, 2)  # bn,l,c
        z = z.flatten(2).transpose(1, 2)
        
        # event_x = event_x.flatten(2).transpose(1, 2)  # bn,l,c
        # event_z = event_z.flatten(2).transpose(1, 2)
        
        x = x + self.pos_embed[:, :self.num_patches_search, :]
        z = z + self.pos_embed[:, self.num_patches_search:, :]

        # event_x = event_x + self.pos_embed[:, :self.num_patches_search, :]
        # event_z = event_z + self.pos_embed[:, self.num_patches_search:, :]

        voxel_z = self.event_encoder(event_template_voxel)
        voxel_x = self.event_encoder(event_search_voxel)

        h_z = w_z = int(np.sqrt(z.shape[1]))
        rgb_template_map = z.reshape(B, h_z, w_z, z.shape[-1])

        h_x = w_x = int(np.sqrt(x.shape[1]))
        rgb_search_map = x.reshape(B, h_x, w_x, x.shape[-1])

        warped_template_feats = []
        warped_search_feats = []
        for i in range(voxel_x.shape[1]):
            warped_template_feat, _ = self.eventfeat_warp(rgb_template_map, voxel_z[:, i])
            warped_search_feat, _ = self.eventfeat_warp(rgb_search_map, voxel_x[:, i])
            warped_template_feats.append(warped_template_feat)
            warped_search_feats.append(warped_search_feat)

        multiscale_template_feat1 = []
        multiscale_search_feat1 = []
        for i in range(len(self.scale1)-1):
            multiscale_template_feat1.append(warped_template_feats[self.scale1[i+1]] - warped_template_feats[self.scale1[i]])
            multiscale_search_feat1.append(warped_search_feats[self.scale1[i+1]] - warped_search_feats[self.scale1[i]])
        multiscale_template_feat1 = torch.stack(multiscale_template_feat1, dim=1)
        multiscale_search_feat1 = torch.stack(multiscale_search_feat1, dim=1)

        template_temporal = multiscale_template_feat1.mean(1).reshape(B, h_z*w_z, -1)
        search_temporal = multiscale_search_feat1.mean(1).reshape(B, h_x*w_x, -1)

        temporal_feat = torch.cat((template_temporal, search_temporal), dim=1)
        cross_out = self.multi_aggblocks(temporal_feat)
        search_temporal = cross_out[:, self.num_patches_template:]
        
        score_wh = self.score_mlp(search_temporal).squeeze(-1)
        
        sorted_score, indices = torch.sort(score_wh, dim=1, descending=True)

        var_wh = score_wh.var(dim=1, unbiased=False)
        
        if var_wh.shape[0] > 1:
            var_norm = (var_wh - var_wh.min()) / (var_wh.max() - var_wh.min() + 1e-6)
        else:
            var_norm = var_wh
            
        K_min, K_max = self.num_patches_search // 2, self.num_patches_search 
        K_adaptive = K_min + (K_max - K_min) * torch.exp(-2 * var_norm)  
        K_adaptive = K_adaptive.mean(0).round().long() 

        topk_idx = indices[:, :K_adaptive]
        non_topk_idx = indices[:, K_adaptive:]
        
        attentive_score = score_wh.gather(dim=1, index=topk_idx).unsqueeze(-1).expand(B, -1, search_temporal.shape[-1])
        attentive_temporal = search_temporal.gather(dim=1, index=topk_idx.unsqueeze(-1).expand(B, -1, search_temporal.shape[-1]))
        attentive_tokens = x.gather(dim=1, index=topk_idx.unsqueeze(-1).expand(B, -1, x.shape[-1]))
  
        xz = torch.cat((z, attentive_tokens), dim=1)
        xz = self.pos_drop(xz)
        ...
        vit_out = self.norm(xz)

        z = vit_out[:, :self.num_patches_template]
        x = vit_out[:, self.num_patches_template:]
        
        x = x + attentive_temporal * attentive_score

        if non_topk_idx is not None:
            pruned_lens_x = self.num_patches_search - K_adaptive
    ...
    def forward(self, z, x, event_template_voxel, event_search_voxel):
        x = self.forward_features(z, x, event_template_voxel, event_search_voxel)
...
def load_pretrained(model, checkpoint, pos_type, patchembed_init):
    if "module" in checkpoint.keys():
        ...
        state_dict = {k.replace("encoder.body.", "", 1): v for k, v in state_dict.items()}
    ...
    if patchembed_init == "copy":
        ...
        # patch_embedding_weight[:,3:,:,:] = patch_embedding_weight_pretrained / 2
        patch_embedding_weight = patch_embedding_weight_pretrained[:,:3,:,:]
...
def fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=3, embed_dim=384, depth_stage1=1, depth_stage2=1, depth=12, num_heads=6, bridge_mlp_ratio=3.,
