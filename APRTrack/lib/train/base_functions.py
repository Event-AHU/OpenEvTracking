...
def build_dataloaders(cfg, settings):
    if cfg.DATA.TRAIN.DATASETS_NAME[0] == "FE108":
        transform_joint = tfm.Transform(tfm.ToGrayscale(probability=0.5))                             
        ...
        transform_train = tfm.Transform(tfm.ToTensor(), tfm.Normalize(mean=cfg.DATA.MEAN, std=cfg.DATA.STD), tfm.RandomHorizontalFlip_Norm(0.5))                                            
        transform_val = tfm.Transform(tfm.ToTensor(), tfm.Normalize(mean=cfg.DATA.MEAN, std=cfg.DATA.STD))
...
def get_optimizer_scheduler(net, cfg):
    param_dicts = [
        ...
        {
            "params": [p for n, p in net.named_parameters() if ("backbone" in n or "apg" in n or "completion" in n) and p.requires_grad],
