...
class Felt(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        super().__init__('Felt', root, image_loader)
        self.sequence_list = self._get_sequence_list()
        if split is not None:
            if seq_ids is not None:
                raise ValueError('Cannot set both split_name and seq_ids.')
            if split == 'train':
                file_path = os.path.join(self.root,'train1k.txt')
            elif split == 'val':
                file_path = os.path.join(self.root, 'val1k.txt')
            elif split == 'test':
                file_path = os.path.join(self.root, 'test1k.txt')
            else:
    ...
    def _get_sequence_list(self):
        with open(os.path.join(self.root, 'list1k.txt')) as f:
    ...
    def _get_event_voxel_sequence_path(self, seq_id): 
        return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel")

    def _get_event_voxel(self, seq_path, frame_id):  
        frame_event_list = []
        for f_id in frame_id:
            ...
            if os.path.getsize(event_frame_file) == 0:  
                ...
                event_features = np.concatenate((mat_data['coor'], mat_data['features']), axis=1) 
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        aps_seq_path = self._get_aps_sequence_path(seq_id)  
        dvs_seq_path = self._get_dvs_sequence_path(seq_id)       
        aps_frame_list = [self._get_frame(aps_seq_path, f_id) for f_id in frame_ids]  
        dvs_frame_list = [self._get_frame(dvs_seq_path, f_id) for f_id in frame_ids] 
        if anno is None:
        ...
        object_meta = OrderedDict({'object_class_name': None,
                                   ...
                                   'root_class': None,
                                   'motion_adverb': None})
        return aps_frame_list, dvs_frame_list, anno_frames, object_meta 