...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        if image.size(0) == 6:
            rgb = tvisf.normalize(image[:3, :, :], self.mean, self.std, self.inplace)
            event = tvisf.normalize(image[3:, :, :], self.mean, self.std, self.inplace)
            return torch.cat((rgb, event), 0)
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            if image.shape[-1] == 6:
                ...
                if (img_rgb == img_event).min() == True:
                    return np.stack([img_gray, img_gray, img_gray, img_gray, img_gray, img_gray], axis=2)
                return np.stack([img_gray, img_gray, img_gray, img_event[:, :, 0], img_event[:, :, 1], img_event[:, :, 2]], axis=2)
