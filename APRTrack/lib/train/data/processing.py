...
class STARKProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
        ...
        args:
            ...
            output_sz - An integer, denoting the size to which the search region is resized. The search region is always square.
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop(frames=data[s + '_images'], box_extract=jittered_anno, box_gt=data[s + '_anno'], search_area_factor=self.search_area_factor[s], output_sz=self.output_sz[s], masks=data[s + '_masks'])

            data[s + '_images'], data[s + '_anno'], data[s + '_att'], data[s + '_masks'] = self.transform[s](image=crops, bbox=boxes, att=att_mask, mask=mask_crops, joint=False)
