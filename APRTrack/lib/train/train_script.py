...
from lib.train.actors import APRTrackActor
...
def run(settings):
    ...
    if settings.script_name == "aprtrack":
        net = build_aprtrack(cfg)
    else:
    ...
    if settings.script_name == "aprtrack":
        ...
        actor = APRTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
