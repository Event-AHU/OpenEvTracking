...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
                 ...
                 distill=None, script_teacher=None, config_teacher=None, resume=None):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='path to a specific checkpoint (.pth.tar) to resume training from'
    )

    args = parser.parse_args()
    import os
    local_rank_env = int(os.environ.get('LOCAL_RANK', -1))
    local_rank = local_rank_env if local_rank_env != -1 else args.local_rank
    if local_rank != -1:
