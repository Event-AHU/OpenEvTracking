...
class Tracker:
    ...
    def run_sequence(self, seq, debug=None):
        ...
        params.debug = debug_
        # 注入序列名和tracker名，供 debug 保存目录使用
        params.seq_name     = getattr(seq, 'name', 'unknown_seq')
        params.tracker_name = self.name
        init_info, start_frame_idx = seq.init_info(0)
        tracker = self.create_tracker(params)
        output  = self._track_sequence(tracker, seq,
                                    start_frame_idx, init_info)
        return output
    ...
    def _track_sequence(self, tracker, seq, start_frame_idx, init_info):
        ...
        event_image = self._read_image(seq.event_img_list[start_frame_idx])
        ...
        out = tracker.initialize(image, event_image, start_frame_idx, init_info)
        if out is None:
            out = {}
        ...
        for frame_num, (frame_path, event_img_path) in enumerate(zip(seq.frames[start_frame_idx+1:], seq.event_img_list[start_frame_idx+1:]), start=start_frame_idx+1):
            ...
            out = tracker.track(image, event_img, info)
            prev_output = OrderedDict(out)
    ...
    def get_parameters(self):
        ...
        try:
            params = param_module.parameters(self.parameter_name, epoch=self.run_id)
        except TypeError:
            # 兼容不支持 epoch 参数的旧 parameter 文件
