...
from lib.models.layers.attn import FlashAttentionLayer
...
class CEBlock(nn.Module):
    """
    CEBlock with FlashAttention backend.
    
    设计决策：
      - FlashAttention 不支持返回 attention map（算法限制）
      - CE 剪枝（keep_ratio < 1）需要 attention map → 退回标准 Attention
      - keep_ratio == 1.0 时（SOR 阶段）全程使用 FlashAttention，不计算 attn map
      - keep_ratio <  1.0 时（未来 CE 激活阶段）自动切换到标准 Attention
    """
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 drop=0., attn_drop=0., drop_path=0., act_layer=nn.GELU,
                 norm_layer=nn.LayerNorm, keep_ratio_search=1.0):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        ...
        # FlashAttention：用于 keep_ratio=1.0 的快速路径
        self.flash_attn = FlashAttentionLayer(
            dim, num_heads=num_heads, qkv_bias=qkv_bias,
            qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop
        )
        # 标准 Attention：用于需要返回 attn map 的 CE 路径（keep_ratio < 1.0）
    ...
    def forward(self, x,
        ...
        """
        统一接口，兼容 vit_ce.py 的 6 参数调用。

        Returns:
            x                  : [B, N, C]  更新后的 token 序列
            global_index_template: [B, N_z]  模板全局索引（不变）
            global_index_search  : [B, N_x]  搜索区全局索引（CE 后可能缩短）
            removed_index_search : [B, N_rm] 被剔除的索引，无剔除时为 None
            attn                 : None（FlashAttn 路径）或 Tensor（标准 Attn 路径）
        """
        #  判断是否需要 CE 剪枝 
        effective_keep = keep_ratio_search if keep_ratio_search is not None \
                         else self.keep_ratio_search
        need_ce = (effective_keep < 1.0) and (global_index_search is not None)

        removed_index_search = None

        if not need_ce:
            #  快速路径：FlashAttention，无需 attn map 
            x_attn = self.flash_attn(self.norm1(x))
            x = x + self.drop_path(x_attn)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            attn = None   # CE 不激活时，调用方不使用 attn

        else:
            #  CE 路径：标准 Attention，返回 attn map 
            x_attn, attn = self.std_attn(self.norm1(x), mask=mask,
