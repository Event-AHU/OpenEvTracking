...
class GaborKernelGenerator(nn.Module):
    ...
    def forward(self, phi_motion:torch.Tensor)->torch.Tensor:
        ...
        # 生成 Gabor 核
        # gb = exp(-0.5 * (x_theta^2 + gamma^2 * y_theta^2) / sigma^2) * cos(2*pi*x_theta/lambd + psi)
        gb = torch.exp(-0.5 * (x_theta**2 + self.gamma**2 * y_theta**2) / self.sigma**2) * \
                torch.cos(2 * math.pi * x_theta/self.lambd + self.psi)
...
class SORModule(nn.Module):
    ...
    def forward(self, f_rgb:torch.Tensor, f_event:torch.Tensor, phi_motion:torch.Tensor)->torch.Tensor:
        """
        Method - forward
        ...
        kernels = self.kernel_gen(phi_motion)
        ...
        f_spatial = self.proj(R_rgb) + f_rgb
