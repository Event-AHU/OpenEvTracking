...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    returns:
        ...
        """
    # box_extract = box_extract * len(frames)
    # box_gt = box_gt * len(frames)
    ...
    if masks is None:
        ...
        crops_resize_factors = [sample_target(f, box_extract[0], search_area_factor, output_sz, masks[0])
                                for f in frames]
    ...
    box_crop = [transform_image_to_crop(box_gt[0], box_extract[0], resize_factors[0], crop_sz, normalize=True)] * len(frames) # (x1,y1,w,h) list of tensors
