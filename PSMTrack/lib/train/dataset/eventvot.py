...
class EventVOT(BaseVideoDataset):
    ...
    def _get_frame_path(self, seq_path, frame_id):
        if os.path.exists(os.path.join(seq_path, '{:04}_2.png'.format(frame_id))):
            return  [os.path.join(seq_path, '{:04}_1.png'.format(frame_id)),
                     os.path.join(seq_path, '{:04}_2.png'.format(frame_id)),
                     os.path.join(seq_path, '{:04}_3.png'.format(frame_id))]   

    def _get_frame(self, seq_path, frame_id):
        return [self.image_loader(img) for img in self._get_frame_path(seq_path, frame_id)]

    # def _get_event_sequence_path(self, seq_id):        ## get events' voxel
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        frame_list = self._get_frame(seq_path, frame_ids[0])     ## Event_img
