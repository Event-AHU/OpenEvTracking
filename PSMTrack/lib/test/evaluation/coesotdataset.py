...
class COESOTDataset(BaseDataset):
    ...
    def __init__(self, split):
        ...
        if split == 'test':
            self.base_path = os.path.join(self.env_settings.coesot_path)
        else:
    ...
    def _construct_sequence(self, sequence_name):
        ...
        frames_path = '{}/{}/{}'.format(self.base_path, sequence_name, sequence_name+'_dvs')
        frame_list = [frame for frame in os.listdir(frames_path) if frame.endswith(".png") or frame.endswith(".bmp") ]
        # frame_list.sort(key=lambda f: int(f[-8:-4]))
        # frames_list = [os.path.join(frames_path, frame) for frame in frame_list]
        # frame_event_list = []
        ...
        for f in frame_list:
            ...
            main_id = int(re.findall(r'\d+', main_id)[0])
            sub_id = int(sub_id)

            groups[main_id].append((sub_id, f))
