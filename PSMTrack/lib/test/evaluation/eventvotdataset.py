...
class EventVOT(BaseDataset):

    def __init__(self, split):
        super().__init__()
        # Split can be test, val, or ltrval (a validation split consisting of videos from the official train set)
        if split == 'test':
            self.base_path = os.path.join(self.env_settings.eventvot_path, 'test_sparse_dense')
        else:
            self.base_path = os.path.join(self.env_settings.eventvot_path, 'train')
    ...
    def _construct_sequence(self, sequence_name):
        ...
        for f in frame_list:
            main_id, sub_id = os.path.splitext(f)[0].split('_')
            groups[int(main_id)].append((int(sub_id), f))

        frames_list = []
        for k in sorted(groups.keys()):
            group = sorted(groups[k], key=lambda x: x[0])
            frames_list.append([
                os.path.join(frames_path, f) for _, f in group
            ])
        ...
        return Sequence(sequence_name, frames_list, 'EventVOT', ground_truth_rect.reshape(-1, 4))
